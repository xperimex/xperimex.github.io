 const s4 = ( sketch ) => {
    ...
    wall_light = new Draggable(400, 400, 6, [255,0,0]);
    wall_target = new Draggable(500, 300, 6, [0,255,0]);
    line1 = new Point(width/2, (600-square_dim)/2);
    line2 = new Point(width/2, 600 - (600-square_dim)/2);
    ...
    sketch.line(0*(wall.x) + line1.x, 0*(wall.y) + line1.y,
        -1*(wall.x) + line1.x, -1*(wall.y) + line1.y);
    
    sketch.stroke(255,0,255)
    sketch.line(0*(wall.x) + line1.x, 0*(wall.y) + line1.y,
        0*(wall.x) + line1.x + square_dim, 0*(wall.y) + line1.y);
    sketch.line(0*(wall.x) + line1.x, 0*(wall.y) + line1.y,
        0*(wall.x) + line1.x - square_dim, 0*(wall.y) + line1.y);
    
    sketch.stroke(0,255,255)
    sketch.line(-1*(wall.x) + line1.x, -1*(wall.y) + line1.y,
        -1*(wall.x) + line1.x + square_dim, -1*(wall.y) + line1.y);
    sketch.line(-1*(wall.x) + line1.x, -1*(wall.y) + line1.y,
        -1*(wall.x) + line1.x - square_dim, -1*(wall.y) + line1.y);

    
    sketch.stroke(100,100,100)
    sketch.line(0*(wall.x) + line1.x + square_dim, 0*(wall.y) + line1.y,
        -1*(wall.x) + line1.x + square_dim, -1*(wall.y) + line1.y);
    sketch.line(0*(wall.x) + line1.x - square_dim, 0*(wall.y) + line1.y,
        -1*(wall.x) + line1.x - square_dim, -1*(wall.y) + line1.y);
    ...
    if(wall_light.x > 300 + square_dim - 5) {
        wall_light.x = 300 + square_dim - 5
    }
    
    if(wall_light.y < (600-square_dim)/2 + 5) {
        wall_light.y = (600-square_dim)/2 + 5
    }
    
    if(wall_light.y > (600 - (600-square_dim)/2 - 5)) {
        wall_light.y = (600 - (600-square_dim)/2 - 5)
    }

    
    if(wall_target.x < 305) {
        wall_target.x = 305
    }
    
    if(wall_target.x > 300 + square_dim - 5) {
        wall_target.x = 300 + square_dim - 5
    }
    
    if(wall_target.y < (600-square_dim)/2 + 5) {
        wall_target.y = (600-square_dim)/2 + 5
    }
    
    if(wall_target.y > (600 - (600-square_dim)/2 - 5)) {
        wall_target.y = (600 - (600-square_dim)/2 - 5)
    }
 ...
 let roomOneWall = new p5(s4, 'roomOneWall');
const s3 = ( sketch2 ) => {
    ...
    sketch2.draw = function() {
        ...
        if (track_mouse) {
            mouse_angle3 = Math.atan2(sketch2.mouseY - light.y, sketch2.mouseX - light.x);
        } else {
            mouse_angle3 = mouse_angle3;
        }
        ...
        sketch2.strokeWeight(1);
        sketch2.stroke(0,0,0)
        sketch2.line(light.x - 1000*Math.cos(mouse_angle3), light.y - 1000*Math.sin(mouse_angle3),
                     light.x + 1000*Math.cos(mouse_angle3), light.y + 1000*Math.sin(mouse_angle3))
 

        var proj = (mirror_center.x - light.x) * Math.cos(mouse_angle3) + (mirror_center.y - light.y) * Math.sin(mouse_angle3);
        proj = new Point(light.x + proj*Math.cos(mouse_angle3), light.y + proj*Math.sin(mouse_angle3))
        // sketch2.stroke(0,0,255);
        // sketch2.line(light.x,light.y,proj.x,proj.y);

        var cross = (mirror_center.x - light.x) * Math.sin(mouse_angle3) - Math.cos(mouse_angle3) * (mirror_center.y - light.y);
...
let crossProduct = new p5(s3, 'crossProduct')
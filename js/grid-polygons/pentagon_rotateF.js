...
// Drawing pentagon
    ...
    for (let j = 0; j < iterations; j++){
        for(let i = 0; i < sides; i++){
            ...
            //          rad * scale * (x(i+1) + Math.sqrt(2) * Math.sin(2 * Math.PI * (i)/sides - Math.PI/4 + phi + angle)), {affects:'none', stroke:'black','stroke-width':5})
    
            // ctx.point(rad * scale * (-y(i) + Math.sqrt(2) * Math.cos(2 * Math.PI * (i-1)/sides - Math.PI/4 + phi + angle)), 
            //           rad * scale * (x(i) + Math.sqrt(2) * Math.sin(2 * Math.PI * (i-1)/sides - Math.PI/4 + phi + angle)), {fill:'black'})
